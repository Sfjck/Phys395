...
real, dimension(:), allocatable :: yFit3svd, yFit7svd, yFit3lss, yFit7lss, fitResults
!Write format labels:
1 format(3f8.3)
2 format(3f8.3,a,1f9.3,a,3f7.3)
3 format(a,1f9.3,3f8.3)
4 format(a,1f15.3)
5 format(a,1f20.3)
6 format(a,1f13.3)
7 format(a,1f9.3,7f8.3)
...
yFit3svd = yFit(x, y, numPoints, 3, fitResults)
...
yFit7svd = yFit(x, y, numPoints, 7, fitResults)
...
write(*,*) "With the above results, the p-value of the chi-squared statistic is 0.250, making the fit poorer. It is probably over-fitted."
deallocate (x, y, fitResults)

!Problem 5:
write(*,*) ""
wirte(*,*) "Problem 5: Fitting noisy data with LSS, n=3. Then computing fit parameters, condition number, and X2. Finally, comparing results to VSD.
end program
